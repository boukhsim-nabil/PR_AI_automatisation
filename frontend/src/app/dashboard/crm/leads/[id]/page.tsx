...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [lead, activities, tasks, assignees] = await Promise.all([
        ...
        <div className="space-y-6">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm" aria-labelledby="contact-title">
            ...
            <dl className="mt-5 grid gap-4 sm:grid-cols-2">
              {[["Email", lead.contact.email], ["Téléphone", lead.contact.phone], ["Organisation", lead.contact.organization_name], ["Fonction", lead.contact.job_title], ["Langue", lead.contact.language], ["Responsable", assignee?.display_name ?? assignee?.email ?? "Non attribué"]].map(([label, value]) => (
                <div key={label}><dt className="text-xs font-semibold uppercase tracking-wider text-slate-400">{label}</dt><dd className="mt-1 break-words text-sm font-medium text-slate-800">{value ?? "Non renseigné"}</dd></div>
              ))}
            </dl>
          </section>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm" aria-labelledby="timeline-title">
            ...
            {!activities?.items.length ? <p className="mt-4 text-sm text-slate-500">Aucune activité.</p> : (
              <ol className="mt-5 space-y-4 border-l-2 border-slate-100 pl-5">
                {activities.items.map((activity) => (
                  <li key={activity.id} className="relative">
                    ...
                    <div className="flex flex-wrap items-baseline justify-between gap-2"><p className="font-semibold text-slate-900">{activity.subject}</p><time className="text-xs text-slate-400">{new Date(activity.occurred_at).toLocaleString("fr-FR")}</time></div>
